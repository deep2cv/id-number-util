...
class IdNumber(str):
    ...
    @classmethod
    def generatex_id(cls, addr_code="230182",sex=1):
        """随机生成身份证号，sex = 0表示女性，sex = 1表示男性"""

        # 随机生成一个区域码(6位数)
        id_number = addr_code#str(random.choice(list(const.AREA_INFO.keys())))
        # 限定出生日期范围(8位数)
        start, end = datetime.strptime("1989-01-01", "%Y-%m-%d"), datetime.strptime("1992-12-31", "%Y-%m-%d")
        birth_days = datetime.strftime(start + timedelta(random.randint(0, (end - start).days + 1)), "%Y%m%d")
        id_number += str(birth_days)
        # 顺序码(2位数)
        id_number += str(random.randint(10, 99))
...
if __name__ == '__main__':
    ...
    rand_id = IdNumber.generatex_id()  # 随机生成身份证号
    print(rand_id)
    print(IdNumber(rand_id).area_id)  # 地址编码:410326
    print(IdNumber(rand_id).get_area_name())  # 地址:河南省洛阳市汝阳县
    print(IdNumber(rand_id).get_birthday())  # 生日:1995-7-10
    print(IdNumber(rand_id).get_age())  # 年龄:23(岁)
    print(IdNumber(rand_id).get_sex())  # 性别:1(男)
    print(IdNumber(rand_id).get_check_digit())  # 校验码:7
    print(IdNumber.verify_id(rand_id))  # 检验身份证是否正确:False
